...
class HandHistoryParser {
  ...
  _runTwiceIndicator(line, lineno) {
    return this._runTwiceIndicatorRx.test(line)
  }
  ...
  _readInfo(line, lineno) {
    ...
    info.zoom = (match[idxs.zoom] != null) ? true : false
                                info.timestamp = (new Date(Date.UTC(info.year, info.month - 1, info.day, info.hour, info.min, info.sec))).getTime()
  ...
  _readSummaryMucked(line, lineno) {
    ...
    const card3  = safeFirstUpper(safeTrim(match[idx++]))
    const card4  = safeFirstUpper(safeTrim(match[idx++]))
  ...
  parse() {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (this._runTwice) {
        ...
        this._runTwice = this._runTwiceIndicator(lines[i], i)
        if (this._runTwice) {
          this.hand.runTwice = true
          continue
        }
      }
